...
if __name__ == "__main__":
    ...
    # part 2
    # the regular expressions really ought to be compiled
    c = 0
    for m in msgs:
        front_reps = 2
        while match_front := re.match(f"^{regex(42, rules)}{{{front_reps}}}", m):
            txt = match_front.group(0)
            rest = m[len(txt):]
            success = False
            for back_reps in range(1, front_reps):
                if match_back := re.match(f"{regex(31, rules)}{{{back_reps}}}$", rest):
                    success = True
                    break
            if success:
                c += 1
